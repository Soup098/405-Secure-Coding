...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    {
        if (result > std::numeric_limits<T>::max() - increment) {//checking if result is greater than the numeric limit of the data type minus the increment, because if it is than an overflow would occur
            return 0;
        }
        else {
            result += increment;
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        ...
        if (result < decrement) {//checking if result is less than decrement, because if it is than subtracting would cause and underflow.
            return 0;
        }
        else {
            result -= decrement;
...
void test_overflow()
{
    ...
    if (result == 0) {
        std::cout << "Test failed, Overflow detected\n";// output for the result that occuurs if an overflow would have occured
    }
    else {
        std::cout << +result << std::endl;
    }
...
void test_underflow()
{
    ...
    if (result == 0) {
        std::cout << "Test has failed, Underflow detected\n";//output for if an underflow were to have occured
    }
    else {
        std::cout << +result << std::endl;
    }
