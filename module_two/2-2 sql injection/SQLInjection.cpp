...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    if (sql.find(" or ") != std::string::npos && //search for if or is found, and then check the following characters to detect possible injection
        sql.find("=") != std::string::npos ||
        sql.find("--") != std::string::npos){
        std::cout << "SQL injection detected." << std::endl;//output error if potential injection found
        return false;
    }

    if (sql.find(';') != sql.rfind(';')) { //search for if a line closing semicolon appears more than once in a statement, detecting possible injection.
        std::cout << "SQL injection detected." << std::endl;//output error if potential injection found
        return false;
    }
